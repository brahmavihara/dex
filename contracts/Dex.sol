...
contract Dex {
    ...
    while(i < orders.length && remaining > 0) {
        ...
        //need to check integer overflow
        if(side == Side.SELL) {
            ...
            traderBalances[msg.sender][DAI] = traderBalances[msg.sender][DAI].sub(matched.mul(orders[i].trader));
        }
        if(side == Side.BUY) {
            require(
            ...
            traderBalances[msg.sender][ticker] = traderBalances[msg.sender][ticker].sub(matched);
            traderBalances[msg.sender][DAI] = traderBalances[msg.sender][DAI].add(matched.mul(orders[i].trader));
        }
    ...
    modifier onlyAdmin() {
        require(msg.sender == onlyAdmin, "only admin");
        _;
    }
}